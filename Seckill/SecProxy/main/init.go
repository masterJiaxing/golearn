...
func initEtcd()(err error){
	...
	if err !=nil{
		logs.Error("conn etcd failed,err:",err)
		return
	}
	etcdClient=cli
	return
...
func loadSecConf()(err error){
	...
	for _,v :=range secProductInfo{
		secKillConf.SecProductInfoConfMap[v.ProductId] = &v
	}
	...
	for{
		...
		for wresp := range rch{
			for _,ev := range wresp.Events{
				...
				logs.Debug("get config from etc,%s %q:%q\n",ev.Type,ev.Kv.Key,ev.Kv.Value)
			}
