...
func parse()  {
	...
	//创建excel文件
	f, err := os.Create("E:/go/goweb/src/golearn/day8/pachong/haha4.xlsx")
	if err != nil {
		panic(err)
	}
	defer f.Close()
	...
	for i:=0;i<2;i++{
		...
		for i:=0;i<len(find_txt2);i++{
			//fmt.Println( find_txt4[i][1],find_txt3[i][1],find_txt2[i][1])
		...
		//  打印全部数据和写入excel文件
		/*for i:=0;i<len(find_txt2);i++{
			//fmt.Println( find_txt4[i][1],find_txt3[i][1])
			fmt.Printf("%s %s %s\n",find_txt4[i][1],find_txt3[i][1],find_txt2[i][1], )
			f.WriteString(find_txt4[i][1]+"\t"+find_txt3[i][1]+"\t"+find_txt2[i][1]+"\t"+"\r\n")
